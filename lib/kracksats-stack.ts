    ...
    constructor(scope?: cdk.Construct, id?: string, props?: cdk.StackProps) {
        ...
        const incomingEmailHandler = new lambda.Function(
            ...
            'IncomingEmailHandler',
            {
                ...
                code: lambda.Code.fromAsset(
                    'lambdas/dist/incomingEmailHandler'
                ),
                handler: 'index.handler',
                environment: {
                    EMAILS_TABLE_NAME: emailsTable.tableName,
        ...
        const withdrawalTransitionHandler = new lambda.Function(
            ...
            'WithdrawalTransitionHandlerLambda',
            {
                ...
                logRetention: 14,
                code: lambda.Code.fromAsset(
                    'lambdas/dist/withdrawalTransitionHandler'
                ),
                handler: 'index.handler',
                environment: {
                    PENDING_WITHDRAWALS_TABLE_NAME:
                        pendingWithdrawalsTable.tableName,
                },
            }
        );
        ...
        const withdrawalPendingEmitter = new lambda.Function(
            ...
            'WithdrawalPendingEmitterLambda',
            {
                ...
                logRetention: 14,
                code: lambda.Code.fromAsset(
                    'lambdas/dist/withdrawalPendingEmitter'
                ),
                handler: 'index.handler',
                environment: {
                    EVENT_BUS_ARN: eventsTopic.topicArn,
        ...
        const incomingEmailsBucket = new s3.Bucket(
            this,
            ...
            rules: [
                {
                    actions: [
                        new sesActions.Lambda({
                            ...
                            invocationType:
                                sesActions.LambdaInvocationType
                                    .REQUEST_RESPONSE,
                        }),
                        new sesActions.S3({
