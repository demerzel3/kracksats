...
class KracksatsStack extends cdk.Stack {
    ...
    constructor(scope, id, props) {
        ...
        const pendingWithdrawalsTable = new dynamodb.Table(this, 'PendingWithdrawals', {
            partitionKey: {
                name: 'refid',
                type: dynamodb.AttributeType.STRING,
            },
        ...
        emailsTable.grant(incomingEmailHandler, 'dynamodb:GetItem', 'dynamodb:PutItem');
        eventsTopic.grantPublish(incomingEmailHandler);

        const incomingEmailParser = new lambda.Function(this, 'IncomingEmailParser', {
            runtime: lambda.Runtime.NODEJS_10_X,
            logRetention: 14,
            code: lambda.Code.asset('lambdas/dist/incomingEmailParser'),
            handler: 'index.handler',
            environment: {
                EVENT_BUS_ARN: eventsTopic.topicArn,
            },
        });
        eventsTopic.addSubscription(new snsSubscriptions.LambdaSubscription(incomingEmailParser, {
            filterPolicy: {
                type: sns.SubscriptionFilter.stringFilter({
        ...
        const withdrawer = new lambda.Function(this, 'WithdrawerLambda', {
            runtime: lambda.Runtime.NODEJS_10_X,
            logRetention: 14,
            code: lambda.Code.asset('lambdas/dist/withdrawer'),
        ...
        eventsTopic.addSubscription(new snsSubscriptions.LambdaSubscription(withdrawer, {
            filterPolicy: {
                type: sns.SubscriptionFilter.stringFilter({
        ...
        const withdrawalChecker = new lambda.Function(this, 'WithdrawalCheckerLambda', {
            runtime: lambda.Runtime.NODEJS_10_X,
            logRetention: 14,
            code: lambda.Code.asset('lambdas/dist/withdrawalChecker'),
            timeout: cdk.Duration.seconds(4),
            handler: 'index.handler',
            environment: {
                KRAKEN_CREDENTIALS_ARN,
        ...
        withdrawalChecker.addEventSource(new lambdaEventSources.SnsEventSource(eventsTopic));
        eventsTopic.grantPublish(withdrawalChecker);

        const withdrawalTransitionHandler = new lambda.Function(this, 'WithdrawalTransitionHandlerLambda', {
            runtime: lambda.Runtime.NODEJS_10_X,
            logRetention: 14,
            code: lambda.Code.asset('lambdas/dist/withdrawalTransitionHandler'),
            handler: 'index.handler',
            environment: {
                PENDING_WITHDRAWALS_TABLE_NAME: pendingWithdrawalsTable.tableName,
            },
        });
        withdrawalTransitionHandler.addEventSource(new lambdaEventSources.SnsEventSource(eventsTopic));
        pendingWithdrawalsTable.grant(withdrawalTransitionHandler, 'dynamodb:PutItem', 'dynamodb:DeleteItem');

        const withdrawalPendingEmitter = new lambda.Function(this, 'WithdrawalPendingEmitterLambda', {
            runtime: lambda.Runtime.NODEJS_10_X,
            logRetention: 14,
            code: lambda.Code.asset('lambdas/dist/withdrawalPendingEmitter'),
            handler: 'index.handler',
            environment: {
                EVENT_BUS_ARN: eventsTopic.topicArn,
                PENDING_WITHDRAWALS_TABLE_NAME: pendingWithdrawalsTable.tableName,
        ...
        const withdrawalPendingTicker = new events.Rule(this, 'WithdrawalPendingTicker', {
            schedule: events.Schedule.cron({
                minute: '*/5',
            }),
        });
        withdrawalPendingTicker.addTarget(new targets.LambdaFunction(withdrawalPendingEmitter));

        const helloRust = new lambda.Function(this, 'HelloRustLambda', {
            runtime: lambda.Runtime.PROVIDED,
            code: lambda.Code.asset('lambdas-rust/dist/hello.zip'),
            handler: 'index.handler',
            environment: {
                KRAKEN_CREDENTIALS_ARN,
            },
