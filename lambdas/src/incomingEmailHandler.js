...
const { EMAILS_TABLE_NAME, EVENT_BUS_ARN } = process.env
...
const publishEmailReceivedEvent = (event) =>
  sns
    .publish({
...
exports.handler = (event, context) => {
  ...
  return s3
    ...
    .then((s3Object) => {
      ...
      return simpleParser(rawEmail).then((parsedEmail) => [
        rawEmail,
        parsedEmail,
      ])
    })
    .then(([rawEmail, parsedEmail]) => {
      ...
      return emailExists(inReplyTo).then((repliedEmailExists) => {
        if (repliedEmailExists) {
          ...
          return storeEmail(EMAILS_TABLE_NAME, emailDetails).then(() =>
            publishEmailReceivedEvent({ email: emailDetails })
          )
        } else {
          // reply with an automatic message
          const reply = `Can't help you with that, please reply to one of my emails directly instead.`

          return buildEmailResponse(parsedEmail, 'kracksats@demerzel3.dev', {
            text: reply,
            html: reply,
