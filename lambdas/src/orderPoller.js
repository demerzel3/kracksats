...
const { KRAKEN_CREDENTIALS_ARN, QUEUE_URL, EVENT_BUS_ARN } = process.env

const unwrapQueueEvent = (event) => {
  const {
    Records: [{ eventSource, body }],
  } = event
  ...
  return parsedBody.Message ? JSON.parse(parsedBody.Message) : parsedBody
}

const publishOrderCompletedEvent = (sns, order) =>
  sns
    .publish({
    ...
    .then((credentials) => {
      ...
      return checkOrderStatus(credentials, orderId)
        .then((order) => {
          ...
          return publishOrderCompletedEvent(sns, order).then(() => order)
        })
        .catch(
          handleError(OrderPendingError, (e) =>
            sqs
              .sendMessage({
