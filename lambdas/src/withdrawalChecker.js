...
const publishWithdrawalTransitionEvent = (event) =>
  sns
    .publish({
...
exports.handler = (event, context) => {
  ...
  const { withdrawal, withdrawalStatus, sourceType, sourceId } = body
  ...
  if (withdrawalStatus === 'pendingExchange') {
    return readSecretJson(KRAKEN_CREDENTIALS_ARN)
      .then((credentials) =>
        fetchWithdrawalTransactionId(credentials, { asset, withdrawalId })
      )
      .then(
        tap((transactionId) => {
          console.log(
            `Withdrawal ${withdrawalId} associated transaction is ${transactionId}`
          )
        })
      )
      .then((transactionId) =>
        publishWithdrawalTransitionEvent({
          withdrawal: {
            ...withdrawal,
            txid: transactionId,
          },
      ...
      .catch(
        handleError(WithdrawalNotFoundError, (e) => console.error(e.message))
      )
      .catch(
        handleError(WithdrawalNoTransactionError, (e) =>
          console.error(e.message)
        )
      )
  } else if (withdrawalStatus === 'pendingChain') {
    return fetchTransactionStatus(transactionId).then((transactionStatus) => {
      console.log(
    ...
    return Promise.resolve(
      `Nothing to do here (withdrawal status: ${withdrawalStatus})`
    )
  }
}
